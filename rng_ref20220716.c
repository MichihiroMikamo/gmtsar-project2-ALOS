...
void rng_ref(void *API, int ranfft, float delr, fcomplex *ref1) {
	...
	printf("pulsedur = %f: read from .PRM as pulse_dur\n", pulsedur);
	printf("npts = %d : = 2.0 * fs * pulsedur\n", npts);
	printf("ts = %f : = 1.0 / (2.0 * fs)\n", ts);
	printf("ranfft = %d\n", ranfft);
	printf("**********************\n");
	...
	k = 0;
	FILE *fp;
	fp = fopen("data/RangeReference.txt", "w");
	...
	FILE *fp2;
	fp2 = fopen("data/RangeReferenceWindow.txt", "w");
	rhww1 = 1.0 - rhww;
	for (i = 0; i < ranfft; i++) {
		if (i < npts) {

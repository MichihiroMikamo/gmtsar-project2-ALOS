...
int main(int argc, char *argv[]) {
	...
	FILE *fcp = NULL, *fcp2 = NULL, *fcp3 = NULL, *fcp4 = NULL, *fcp5 = NULL;
	int ranfft;
	...
	printf("**********************\n");
	printf("fs = %f : read from .PRM as rng_samp_rate\n",fs);
	printf("delr = %f : delr = SOL / fs / 2.0 \n", delr);
	printf("near_range = %f : near_range + (st_rng_bin - nextend + xshift - 1) * delr) \n", near_range);
	printf("far_range  = %f : near_range + delr * (num_rng_bins - 1) \n", far_range);
	printf("**********************\n");
	...
	if ((fcp = fopen("data/CopyData_indata.raw", "wb")) == NULL) {
		fprintf(stderr, "Bad output file pointer.\n");
		exit(-1);
	}
	if ((fcp2 = fopen("data/CopyData_RangeCompressed.raw", "wb")) == NULL) {
		fprintf(stderr, "Bad output file pointer.\n");
		exit(-1);
	}
	...
	for (ipatch = 1; ipatch <= num_patches; ipatch++) {
		...
		printf("**********************\n");
		printf("nrows = %d : read from .PRM\n", nrows);
		printf("num_rng_bins = %d\n", num_rng_bins);
		printf("ranfft = %d : = fft_bins(num_rng_bins) \n", ranfft);
		printf("xshift = %d : read from .PRM as r_shift\n", xshift);
		printf("**********************\n");
		...
		for (k = 0; k < nrows; k++) {
			...
			if (ineg >= 0) {
				...
				// copy input data //
				/////////////////////
				fwrite((void *)indata, 2 * sizeof(unsigned char), good_bytes / 2, fcp);
			...
			////////////////////////////////
			// copy range compressed data //
			////////////////////////////////
			fwrite((void *)fft_vec, sizeof(fcomplex), ranfft, fcp2);
		...
		} // for (k = 0; k < nrows; k++) end

		
		//////////////////////////////
		/// output for each patch ////
		//////////////////////////////
		char fdatafilename[1024];
		sprintf(fdatafilename, "data/fdata_ipatch%d.raw", ipatch);
		fcp3 = fopen(fdatafilename, "wb");
		for(int k = 0; k < nrows; k++){
			fwrite((void *)fdata[k], sizeof(fcomplex), num_rng_bins, fcp3);
		}
		//////////////////////////////
		/// output for each patch ////
		//////////////////////////////
		...
		/// output for each patch ////
		//////////////////////////////
		char fdatafilename3[1024];
		sprintf(fdatafilename3, "data/fdata_ipatch%d_after_acpatch.raw", ipatch);
		fcp4 = fopen(fdatafilename3, "wb");
		for(int k = 0; k < nrows; k++){
			fwrite((void *)fdata[k], sizeof(fcomplex), num_rng_bins, fcp4);
		}
		//////////////////////////////
		/// output for each patch ////
		//////////////////////////////
		...
		printf("***************************\n");
		printf("num_valid_az = %d : read from .PRM\n", num_valid_az);
		printf("low_ind = %d : (nrows - num_valid_az) / 2\n", low_ind);
		printf("nrows / 2 = %d\n", (int)(nrows / 2));
		printf("hi_ind = %d : (nrows + num_valid_az) / 2\n", hi_ind);
		printf("a_stretch_r = %f : read from .PRM\n", a_stretch_r);
		printf("a_stretch_a = %f : read from .PRM\n", a_stretch_a);
		printf("***************************\n");
		...
		}// for (i = low_ind; i < hi_ind; i += nlooks) end
		...
		char fdatafilename2[1024];
		sprintf(fdatafilename2, "data/fdata_ipatch%d_low_to_high.raw", ipatch);
		fcp4 = fopen(fdatafilename2, "wb");
		for(int i = low_ind; i < hi_ind; i += nlooks){
			fwrite((void *)fdata[i], sizeof(fcomplex), num_rng_bins, fcp4);
		}
		//////////////////////////////
		/// output for each patch ////
		//////////////////////////////

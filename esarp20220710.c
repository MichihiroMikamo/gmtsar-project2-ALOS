...
int main(int argc, char *argv[]) {
	...
	if ((fcp = fopen("data/CopyData_indata.raw", "w")) == NULL) {
		fprintf(stderr, "Bad output file pointer.\n");
		exit(-1);
	}
	if ((fcp2 = fopen("data/CopyData_RangeCompressed.raw", "w")) == NULL) {
		fprintf(stderr, "Bad output file pointer.\n");
		exit(-1);
	}
